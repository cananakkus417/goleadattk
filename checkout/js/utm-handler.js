...
function initUTMHandler(hardCodedConfig) {
    const config = {
        ...
        'apiEndpoint': 0, // API de tracking externa removida
    };
    ...
    async function dispatch(data) {
        ...
        async function run() {
            // API de tracking externa desabilitada
            return;
            /* CÓDIGO REMOVIDO - Tracking externo desabilitado
    ...
    async function onMount() {
        // onLoad(sendTrackingData); // Removido - API de tracking externa desabilitada
        initializeFromScript();
        initFingerPrint();
        await onLoad(handleUtmParameters);
        initWatch();
    }
    // Função de tracking externa removida (Traki.io/XTracky)
